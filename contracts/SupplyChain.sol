...
contract SupplyChain {
  ...
  constructor() public {
    ...
    modifier forSale(uint sku) {
        require(items[sku].state == State.ForSale, "item not for not for sale");
        _;
    }
    // modifier sold(uint _sku)
    modifier sold(uint sku) {
        require(items[sku].state == State.Sold, "item already sold out");
        _;
    }
    // modifier shipped(uint _sku)
    modifier shipped(uint sku) {
        require(items[sku].state == State.Shipped, "item already shipped");
        _;
    }
    // modifier received(uint _sku)
    modifier received(uint sku) {
        require(items[sku].state == State.Received, "item already received");
        _;
    }
